...
-   `--numa mirror`: Duplicate the model weights and KV cache once per NUMA node and pin threads per node so each node only ever reads its own local copy, eliminating cross-node traffic for the read-mostly data. This lets a multi-socket machine use all sockets productively (especially for token generation, which is memory-bandwidth bound) at the cost of using N&times; the RAM (N = number of NUMA nodes). Implies `--no-mmap`. Use `--numa-mirror <list>` (`weights`, `kv`, `all`, `none`) to customize what is mirrored (default `all`).
